...
MVP читает новые посты из публичных Telegram-каналов через веб-страницы `https://t.me/s/<channel>`, а также может забирать вакансии с `hh.ru` через официальный API поиска вакансий и с `jobs.ton.org` через web scraping. Затем он определяет PM-вакансии, считает fit с резюме и отправляет подходящие посты в целевой канал.
...
Для `https://jobs.ton.org/jobs` используется web scraping listing page и detail pages только для новых вакансий. Дедупликация идет по `job id` из URL и по хешу текста.

Параметры TON board находятся в секции `ton_jobs`:
...
- `min_salary_rur`: не публиковать вакансии с зарплатой ниже порога в рублях
- `require_remote`: публиковать только вакансии с удаленкой / remote / hybrid
- `allowed_domain_keywords`: публиковать только вакансии из нужных доменов
- `excluded_grade_terms`: отсеивать junior / intern роли
- `excluded_company_keywords`: отсеивать аутсорс, аутстафф и заказную разработку
